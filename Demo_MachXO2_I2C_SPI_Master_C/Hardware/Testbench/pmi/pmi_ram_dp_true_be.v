...
// --------------------------------------------------------------------
//
// PMI Simulation File for True Dual Port Block RAM with Byte Enable
//
...
pmi_byte_size                  8|9                                          9
pmi_family  "XP2"|"SC"|"SCM"|"ECP2"|"ECP2M"|"ECP3"|"ECP4"|"XO2"         "ECP2"
------------------------------------------------------------------------------
...
*/
//
// fpga\verilog\pkg\versclibs\data\pmi\pmi_ram_dp_true_be.v 1.0 17-JAN-2011 12:26:27 IALMOHAN
...
//Define internal Signals
   ...
   reg WrA_reg = 0;
   reg WrB_reg = 0;
   reg [((pmi_data_width_a+pmi_byte_size-1)/pmi_byte_size-1):0] ByteEnA_reg = 0;
   reg [((pmi_data_width_b+pmi_byte_size-1)/pmi_byte_size-1):0] ByteEnB_reg = 0;
   ...
   reg WrA_reg_async = 0;
   reg WrB_reg_async = 0;
   reg [((pmi_data_width_a+pmi_byte_size-1)/pmi_byte_size-1):0] ByteEnA_reg_async = 0;
   reg [((pmi_data_width_b+pmi_byte_size-1)/pmi_byte_size-1):0] ByteEnB_reg_async = 0;
   ...
   reg WrA_reg_sync = 0;
   reg WrB_reg_sync = 0;
   reg [((pmi_data_width_a+pmi_byte_size-1)/pmi_byte_size-1):0] ByteEnA_reg_sync = 0;
   reg [((pmi_data_width_b+pmi_byte_size-1)/pmi_byte_size-1):0] ByteEnB_reg_sync = 0;
...
//Read-before-Write is supported for memory configurations in x9, x18, x36 datawidth. Data width should be set to match this requirement.
   ...
   initial begin
      if ((pmi_family != "SC") && (pmi_family != "SCM") && (pmi_family != "XO") &&
          (pmi_family != "LPTM") && (pmi_write_mode_a == "readbeforewrite"))
	begin
      ...
      if ((pmi_family != "SC") && (pmi_family != "SCM") && (pmi_family != "XO") &&
          (pmi_family != "LPTM") && (pmi_write_mode_b == "readbeforewrite"))
	begin
...
initial begin
	if (pmi_data_width_a <= pmi_byte_size || pmi_data_width_b <= pmi_byte_size) begin
		$display("\nError! Byte Enable feature is not supported for single-byte data widths!");
		$stop;
	end
	if (pmi_data_width_a != pmi_data_width_b) begin
		$display("\nError! Data Widths for Ports A and B must be the same for Byte Enable support!");
		$stop;
	end
end
...
//Choice between async/sync resetmode
   ...
   always @(DataInA_reg_sync, AddressA_reg_sync, WrA_reg_sync, ByteEnA_reg_sync, ByteEnA_reg_async,
	         DataInA_reg_async, AddressA_reg_async, WrA_reg_async, QA_reg_async, QA_reg_sync) begin
      if (pmi_resetmode == "async")
	begin
	   DataInA_reg = DataInA_reg_async;
	   AddressA_reg = AddressA_reg_async;
	   WrA_reg = WrA_reg_async;
	   ByteEnA_reg = ByteEnA_reg_async;
	   QA_reg = QA_reg_async;
	end
      else
	begin
   ...
   always @(DataInB_reg_sync, AddressB_reg_sync, WrB_reg_sync, ByteEnB_reg_sync, ByteEnB_reg_async,
	         DataInB_reg_async, AddressB_reg_async, WrB_reg_async, QB_reg_async, QB_reg_sync) begin
      if (pmi_resetmode == "async")
	begin
	   DataInB_reg = DataInB_reg_async;
	   AddressB_reg = AddressB_reg_async;
	   WrB_reg = WrB_reg_async;
	   ByteEnB_reg = ByteEnB_reg_async;
	   QB_reg = QB_reg_async;
	end
      else
	begin
   ...
   //always @(WrA_reg, WrB_reg, AddressA_reg, AddressB_reg, DataInA_reg, DataInB_reg, ClockA_valid, ClockB_valid)
	...
	begin
		...
		begin
			...
			for (n = 0; n < pmi_data_width_a; n = n+1)
				if (ByteEnA_reg[n / pmi_byte_size] == 1'b1 || pmi_data_width_a <= pmi_byte_size)
			begin
				if ( (wr_a_wr_b_coll == 1) &&
					  ((v_WADDR_A * pmi_data_width_a + n) >= dn_coll_addr) &&
					  ((v_WADDR_A * pmi_data_width_a + n) <= up_coll_addr) &&
					  (ByteEnB_reg[((v_WADDR_A * pmi_data_width_a + n) % pmi_data_width_b) / pmi_byte_size] == 1'b1) )
		...
		begin
			...
			for (k = 0; k < pmi_data_width_b; k = k+1)
				if (ByteEnB_reg[k / pmi_byte_size] == 1'b1 || pmi_data_width_b <= pmi_byte_size)
			begin
				if ( (wr_a_wr_b_coll == 1) &&
					  ((v_WADDR_B * pmi_data_width_b + k) >= dn_coll_addr) &&
					  ((v_WADDR_B * pmi_data_width_b + k) <= up_coll_addr) &&
					  (ByteEnA_reg[((v_WADDR_B * pmi_data_width_b + k) % pmi_data_width_a) / pmi_byte_size] == 1'b1) )
				...
				if (last_ClockA_valid == 0)
					...
					begin
						if ( (rd_a_wr_b_coll == 1) &&
							  ...
							  (ByteEnB_reg[((v_RADDR_A * pmi_data_width_a + l) % pmi_data_width_b) / pmi_byte_size] == 1'b1) )
							QA_int[l] <= 1'bX;
						else
			...
			begin //Normal
				if (last_ClockB_valid == 0)
					...
					begin
					  ...
					  ((ByteEnA_reg[((v_RADDR_B * pmi_data_width_b + m) % pmi_data_width_a) / pmi_byte_size] == 1'b1) || pmi_data_width_a <= pmi_byte_size) )
							QB_int[m] <= 1'bX;
						else
