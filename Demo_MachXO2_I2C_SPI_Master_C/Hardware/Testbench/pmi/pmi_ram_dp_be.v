...
// --------------------------------------------------------------------
//
// PMI Simulation File for Dual Port Block RAM with Byte Enable
//
...
*/
//
// fpga\verilog\pkg\versclibs\data\pmi\pmi_ram_dp_be.v 1.1 19-JAN-2011 11:23:41 IALMOHAN
...
//Define internal Signals
   ...
   reg WE_reg = 0;
   reg [((pmi_wr_data_width+pmi_byte_size-1)/pmi_byte_size-1):0] ByteEn_reg = 0;
   ...
   reg WE_reg_async = 0;
   reg [((pmi_wr_data_width+pmi_byte_size-1)/pmi_byte_size-1):0] ByteEn_reg_async = 0;
   ...
   reg WE_reg_sync = 0;
   reg [((pmi_wr_data_width+pmi_byte_size-1)/pmi_byte_size-1):0] ByteEn_reg_sync = 0;
...
//Module Manager supports 1, 2, 4, 8, 16 and 32 only.

initial begin
   if (pmi_wr_data_width > pmi_rd_data_width)
     begin
	  if ((pmi_wr_data_width / pmi_rd_data_width) > 32)
	    begin
	       $display("\nError! The ratio of Write and Read Data width values can not be greater than 32!");
	       $stop;
	    end
	  else if ( (pmi_wr_data_width / pmi_rd_data_width !== 2) && (pmi_wr_data_width / pmi_rd_data_width !== 4) && (pmi_wr_data_width / pmi_rd_data_width !== 8) &&
               (pmi_wr_data_width / pmi_rd_data_width !== 16) && (pmi_wr_data_width / pmi_rd_data_width !== 32) )
	    begin
     ...
     begin
	if ((pmi_rd_data_width / pmi_wr_data_width) > 32)
	    begin
	       $display("\nError! The ratio of Read and Write Data width values can not be greater than 32!");
	       $stop;
	    end
	  else if ( (pmi_rd_data_width / pmi_wr_data_width !== 2) && (pmi_rd_data_width / pmi_wr_data_width !== 4) && (pmi_rd_data_width / pmi_wr_data_width !== 8) &&
               (pmi_rd_data_width / pmi_wr_data_width !== 16) && (pmi_rd_data_width / pmi_wr_data_width !== 32) )
	    begin
...
initial begin
	if (pmi_wr_data_width <= pmi_byte_size) begin
		$display("\nError! Byte Enable feature is not supported for single-byte data widths!");
		$stop;
	end
	if (pmi_rd_data_width > 2*pmi_wr_data_width) begin
		$display("\nError! Read Data Width cannot be more than double Write Data Width for Byte Enable support!");
		$stop;
	end
	if ( ( pmi_rd_data_width / pmi_wr_data_width != ((pmi_rd_data_width+pmi_byte_size-1)/pmi_byte_size) / ((pmi_wr_data_width+pmi_byte_size-1)/pmi_byte_size) ) ||
        ( pmi_wr_data_width / pmi_rd_data_width != ((pmi_wr_data_width+pmi_byte_size-1)/pmi_byte_size) / ((pmi_rd_data_width+pmi_byte_size-1)/pmi_byte_size) ) ) begin
		$display ("\nError! The ratio of write/read bits must equal the ratio of write/read bytes!");
		$stop;
	end
end
...
//Choice between async/sync resetmode
   ...
   always @(Data_reg_sync, WrAddress_reg_sync, WE_reg_sync, ByteEn_reg_sync,
	         Data_reg_async, WrAddress_reg_async, WE_reg_async, ByteEn_reg_async) begin
	...
	begin
		...
		begin
			for (n = 0; n < pmi_wr_data_width; n = n+1)
				if (ByteEn_reg[n / pmi_byte_size] == 1'b1)
		...
		begin
			...
			begin
				if ( (wr_rd_coll == 1) &&
					  ...
					  (ByteEn_reg[((v_RADDR * pmi_rd_data_width + m) % pmi_wr_data_width) / pmi_byte_size] == 1'b1) )
					Q_int[m] <= 1'bX;
				else
